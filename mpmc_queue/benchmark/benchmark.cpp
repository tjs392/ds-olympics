...
template <typename T>
void benchmark_sharded_mpmc(const std::string& name,
                            int num_producers,
                            int num_consumers,
                            size_t items_per_producer) {
    const size_t total_items = num_producers * items_per_producer;

    ShardedMPMCQueue<T> q(num_producers, items_per_producer);
        ...
        consumers.emplace_back([&, c]() {
            ...
            while (consumed_total.load(std::memory_order_relaxed) < total_items) {
                ...
                if (q.pop(val)) {
                    ...
                    if constexpr (std::is_same_v<T, SmallObject>) {
                        stats.dummy += val.i +
                                       static_cast<size_t>(val.d) +
                                       static_cast<size_t>(val.f);
                    } else {
    ...
    size_t total_ops =
        std::accumulate(producer_stats.begin(), producer_stats.end(), 0ull,
...
int main() {
    ...
    for (auto& [p, c] : configs) {
        benchmark_sharded_mpmc<int>("int", p, c, items_per_producer);
        benchmark_sharded_mpmc<SmallObject>("SmallObject", p, c, items_per_producer);
    }
